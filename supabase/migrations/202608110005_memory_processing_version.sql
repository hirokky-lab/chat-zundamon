-- Receipts written before atomic mutation/receipt commits are ambiguous: their
-- pending and failed states must never be replayed automatically.
alter table public.memory_processing
  add column if not exists processing_version smallint not null default 1
...
declare
  ...
  current_attempted_at timestamptz;
begin
  ...
  values (current_user_id, p_source_message_id, 'pending', pg_catalog.now(), null, 2)
  on conflict (user_id, source_message_id) do nothing;
  if found then return 'claimed'; end if;

  select state, processing_version, attempted_at into current_state, current_version, current_attempted_at
  ...
  if current_version <> 2 or current_state <> 'pending' then return 'quarantined'; end if;
  if current_attempted_at > pg_catalog.now() - interval '5 minutes' then return 'pending'; end if;

  update public.memory_processing
  set attempted_at = pg_catalog.now(), completed_at = null
  where user_id = current_user_id and source_message_id = p_source_message_id and processing_version = 2 and state = 'pending';
  return 'claimed';
end;
$$;
...
begin
  ...
  insert into public.memory_processing (user_id, source_message_id, state, attempted_at, completed_at, processing_version)
  values (current_user_id, p_source_message_id, 'pending', now_at, null, 2)
  on conflict (user_id, source_message_id) do nothing;

  if not found then
    select state, attempted_at, processing_version into receipt_state, receipt_attempted_at, receipt_version
    from public.memory_processing
    where user_id = current_user_id and source_message_id = p_source_message_id
    for update;
    if receipt_state = 'completed' then return 'completed'; end if;
    if receipt_version <> 2 or receipt_state <> 'pending' then return 'quarantined'; end if;
    if receipt_attempted_at > now_at - interval '5 minutes' then return 'pending'; end if;
    update public.memory_processing
    set state = 'pending', attempted_at = now_at, completed_at = null
    where user_id = current_user_id and source_message_id = p_source_message_id and processing_version = 2;
  end if;
  ...
  if action_type in ('add', 'replace') then candidate := p_action->'candidate';
  elsif action_type = 'mark_past' then candidate := p_action->'replacement';
  end if;
  ...
  if candidate is not null and jsonb_typeof(candidate) <> 'null' then
    ...
    if exists (select 1 from public.memory_tombstones where user_id = current_user_id and normalized_fingerprint = candidate->>'normalizedContent' and released_at is null) then
      raise exception 'memory candidate blocked';
    end if;
  end if;
  ...
  if action_type = 'add' then
    ...
    perform 1 from public.memories where user_id = current_user_id and id = target_id and status in ('active', 'uncertain') for update;
    if not found then raise exception 'memory target unavailable'; end if;
    update public.memories set status = 'past', updated_at = now_at where user_id = current_user_id and id = target_id;
    if candidate is not null and jsonb_typeof(candidate) <> 'null' then
      insert into public.memories (
        user_id, id, kind, scope, content, content_normalized, status, origin, sensitivity, importance,
        source_message_id, source_occurred_at, valid_from, valid_until, expires_at, pinned, supersedes_id, created_at, updated_at
    ...
    elsif action_type = 'replace' then raise exception 'invalid memory candidate'; end if;
  elsif action_type = 'mark_uncertain' then
    if jsonb_typeof(p_action->'targetMemoryIds') <> 'array' or jsonb_array_length(p_action->'targetMemoryIds') not between 1 and 3 then raise exception 'invalid memory targets'; end if;
    select count(*), count(distinct item.value) into target_count, distinct_target_count from jsonb_array_elements_text(p_action->'targetMemoryIds') item(value);
    if target_count <> distinct_target_count then raise exception 'invalid memory targets'; end if;
    update public.memories set status = 'uncertain', updated_at = now_at
    where user_id = current_user_id and status in ('active', 'uncertain') and id::text in (select value from jsonb_array_elements_text(p_action->'targetMemoryIds') item(value));
    ...
    if not (p_action ? 'targetMemoryId') or p_action->>'blockRelearning' not in ('true','false') then raise exception 'invalid memory target'; end if;
    target_id := (p_action->>'targetMemoryId')::uuid;
    ...
    if (p_action->>'blockRelearning')::boolean then
      ...
      select current_user_id, pg_catalog.gen_random_uuid(), id, content_normalized, now_at, null from public.memories where user_id = current_user_id and id = target_id;
    end if;
    delete from public.memories where user_id = current_user_id and id = target_id;
  end if;

  update public.memory_processing set state = 'completed', completed_at = now_at
  where user_id = current_user_id and source_message_id = p_source_message_id and processing_version = 2;
  return 'applied';
end;
$$;
